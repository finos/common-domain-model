...
=======
Many different financial taxonomies may be used by various segments of the financial industry to describe the same product. To support a multitude of taxonomies with adding and specific identity to data types in the model, a Taxonomy type is used to identify the source and attributes any particular taxonomy structure.
>>>>>>> fe61dbde2 (icma-documentation-update)
...
Repo Product in JSON Format
^^^^^^^^^^^^^^^^^^^^^^^^^^^

A fully formed repo product in json format is described as:
