...
As noted earlier in this document, product qualification is inferred from the economic terms in a primitive event rather than explicitly naming the product type.  The CDM uses a set of Product Qualification functions to achieve this purpose.  These functions can be identified as those with the annotation [qualification Product].

A Product Qualification function applies product specific business logic to identify if the populated values in the primitive event match the specified criteria for the product named in that function. For example, if a certain set of attributes are populated and others are absent, then a specific product can be inferred. 

The set of CDM Product Qualification functions represent the full scope of the ISDA Product Taxonomy v1.0, and the names of the functions match with the lowest level detail in this taxonomy.  Each Product Qualification function name in the CDM begins with the word "qualify" followed by an underscore and then the ISDA taxonomy name, separated by underscores.  An example is provided below for the qualification of a Zero-Coupon Fixed-Float Inflation Swap.
...
If all the statements above are true, then the function evaluates to True.  In this case, the defined product in the primitive event is determined to be qualified as the product referenced by the function name.  As illustrated above, this name matches the corresponding synonym from the ``ISDA_Taxonomy_v1``, which is displayed under the output. Additionally, the function generates a ``productIdentification`` object is populated with this value in the ``productQualifier`` attribute.

The ``productIdentification`` data structure and an example of the output are shown below:
...
For a further understanding of the underlying qualification logic in the Product Qualification, see the explanation of the *object qualification* feature of the Rosetta DSL, as described in the `Function Definition Section`_ -- this link did not take me to this section
