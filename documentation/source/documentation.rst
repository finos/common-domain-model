...
* The full scope of for Interest Rate Products has been represented, down to the full level of detail in the taxonomy.
* Credit Default products are qualified, but not down to the full level of detail. The ISDA taxonomy v2.0 references the transaction type instead of the product feature, whose possible values are not publicly available and hence not positioned as a CDM enumeration.
* Only a small number of products for Foreign Exchange have been qualified.

As illustrated above, the corresponding taxonomy name under the ISDA Product Taxonomy v1.0 may also be displayed as a ``synonym`` under the output.

Additionally, the qualification function's output is used to populate the ``productQualifier`` attribute of the ``productIdentification`` object, which is created when a ``ContractualProduct`` object is created. The product identification includes both the product qualification generated by the CDM and any product identification information which may come from the originating document (such as FpML).

The ``productIdentification`` data structure and an example of the (serialised) JSON output are shown below:
