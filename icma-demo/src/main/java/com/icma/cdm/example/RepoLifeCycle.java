...
import com.rosetta.model.lib.mapper.*;
...
public class RepoLifeCycle {
	...
	public String OnDemandRateChange (
		...
		BigDecimal newAgreedRate = new BigDecimal(agreedRate);

		ObjectMapper rosettaObjectMapper = RosettaObjectMapper.getNewRosettaObjectMapper();
		TradeState tradeStateObj = new TradeState.TradeStateBuilderImpl();
		TradeState tradeState  = rosettaObjectMapper.readValue(tradeStateStr, tradeStateObj.getClass());

		AdjustableOrRelativeDate effectiveRateDate = ru.createAdjustableOrRelativeDate(effectiveDateStr);

		PrimitiveInstruction pi = cf.evaluate(tradeState, effectiveRateDate, newAgreedRate);
	...
	public String OnDemandInterestPayment (
			String tradeStateStr,
		...
		Create_OnDemandInterestPaymentPrimitiveInstruction cf =  new Create_OnDemandInterestPaymentPrimitiveInstruction.Create_OnDemandInterestPaymentPrimitiveInstructionDefault();
		Injector injector = Guice.createInjector(new CdmRuntimeModule());
		injector.injectMembers(cf);

		BigDecimal interestAmount = new BigDecimal(interestAmountStr);

		Money interestMoney = Money.builder()
		...
		SettlementDate settlementDate = SettlementDate.builder()
				.setAdjustableOrRelativeDate(ru.createAdjustableOrAdjustedOrRelativeDate(effectiveDateStr))
				.build();

		PrimitiveInstruction pi = cf.evaluate(tradeState, interestMoney, settlementDate);
