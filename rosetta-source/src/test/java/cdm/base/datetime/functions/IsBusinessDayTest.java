...
public class IsBusinessDayTest extends AbstractFunctionTest {
    ...
    void shouldGetResult() {
        ...
        assertEquals(Boolean.TRUE, func.evaluate(Date.of(2021,12, 20), londonTargetBc));
        assertEquals(Boolean.FALSE, func.evaluate(Date.of(2021,12, 19), londonTargetBc));
        assertEquals(Boolean.FALSE, func.evaluate(Date.of(2021,12, 27), londonTargetBc));
        assertEquals(Boolean.FALSE, func.evaluate(Date.of(2021,12, 28), londonTargetBc));

        List<String> targetBc = getBusinessCenters(TARGET_BC);
        assertEquals(Boolean.TRUE, func.evaluate(Date.of(2021,12, 27), targetBc));
    }
