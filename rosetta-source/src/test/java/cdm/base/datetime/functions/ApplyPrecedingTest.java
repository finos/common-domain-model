...
public class ApplyPrecedingTest extends AbstractFunctionTest {

    @Inject
    private ApplyPreceding func;

    @Test
    void shouldReturnSameDateWhenNotAHoliday() {
        List<Date> holidays = Arrays.asList(Date.of(2024, 1, 15));
        Date result = func.evaluate(Date.of(2024, 1, 16), holidays);
        assertEquals(Date.of(2024, 1, 16), result);
    }

    @Test
    void shouldReturnPreviousDayWhenDateIsHoliday() {
        List<Date> holidays = Arrays.asList(Date.of(2024, 1, 15));
