...
public class FunctionInputCreator {
    ...
    private CreateBusinessEventInput getQuantityChangeFuncInputJson(TradeState tradeState, Date eventDate, QuantityChangeInstruction quantityChangeInstruction, UnscheduledTransferEnum feeType) throws IOException {
        Instruction instructionBuilder = Instruction.builder()
                .setBeforeValue(tradeState)
                .setPrimitiveInstruction(PrimitiveInstruction.builder()
    ...
    private TransferInstruction.TransferInstructionBuilder getTransferInstruction(TradeState tradeState, UnscheduledTransferEnum feeType) {
        ...
        UnitType currencyUnitType = trade.getTradeLot().stream()
                ...
                .filter(unit -> ((UnitType) unit).getCurrency() != null)
                .findFirst()
                .orElse(null);
...
//                                                        .setNameValue("VOLKSWAGEN AG VZO O.N.")))))
    ...
    private TradeState.TradeStateBuilder getTerminationVanillaSwapTradeState() throws IOException {
        ...
        tradeStateBuilder.getTrade().getTradeLot().stream()
                ...
                .forEach(quantity -> ((NonNegativeQuantitySchedule.NonNegativeQuantityScheduleBuilder) quantity).setValue(new BigDecimal(10000)));
        // trade id
